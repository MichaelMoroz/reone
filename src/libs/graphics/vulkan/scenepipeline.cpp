...
void VulkanScenePipeline::shadowPass(VkCommandBuffer cmd,
    ...
    bool isDirectional = _shadow == VulkanSceneShadow::Directional;
    ...
    VulkanRenderPass recorder(_options,
                                        _renderer.device(),
                                        _renderer.pipelines(),
    ...
    recorder.setGlobalsOffset(globalsOffset);
    recorder.setShadowViewMask(viewMask);
    callbacks.draw(isDirectional ? VulkanSceneDraw::DirectionalShadow
                                 : VulkanSceneDraw::PointShadow,
                   recorder);
...
void VulkanScenePipeline::geometryPass(VkCommandBuffer cmd, uint32_t globalsOffset,
                                       IVulkanSceneCallbacks &callbacks) {
    if (!_options.pbr) {
        retroGeometryPass(cmd, globalsOffset, callbacks);
        return;
    }
    ...
    VulkanRenderPass recorder(_options,
                                        ...
                                        VulkanGBuffer::depthFormat());
    recorder.setGlobalsOffset(globalsOffset);
    callbacks.draw(VulkanSceneDraw::Opaque, recorder);

    vkCmdEndRendering(cmd);
}

void VulkanScenePipeline::retroGeometryPass(VkCommandBuffer cmd, uint32_t globalsOffset,
                                            IVulkanSceneCallbacks &callbacks) {
    ...
    vkCmdSetViewport(cmd, 0, 1, &viewport);
    vkCmdSetScissor(cmd, 0, 1, &scissor);
    VulkanRenderPass recorder(
        _options, _renderer.device(), _renderer.pipelines(), _renderer.uniformRing(),
        _renderer.descriptors(), _renderer.resources(), _uniforms, _renderer.pbrTextures(),
        _meshRegistry, cmd,
        {_renderer.swapchain().imageFormat(), _renderer.swapchain().imageFormat()},
        VulkanGBuffer::depthFormat(), VulkanRenderPass::Kind::Retro);
    recorder.setGlobalsOffset(globalsOffset);
    callbacks.draw(VulkanSceneDraw::RetroOpaque, recorder);
    vkCmdEndRendering(cmd);
 ...
 * to know this ran.
 */
void VulkanScenePipeline::hilightsBlurPass(VkCommandBuffer cmd) {
...
void VulkanScenePipeline::screenSpaceEffectsPass(VkCommandBuffer cmd, uint32_t globalsOffset) {
...
void VulkanScenePipeline::resolvePass(VkCommandBuffer cmd, uint32_t globalsOffset) {
 ...
 */
void VulkanScenePipeline::drawOntoOutput(VkCommandBuffer cmd,
                                          uint32_t globalsOffset,
                                          VulkanSceneDraw draw,
                                          const char *label,
                                          IVulkanSceneCallbacks &callbacks) {
 ...
 * unchanged.
 */
void VulkanScenePipeline::transparencyPass(VkCommandBuffer cmd, uint32_t globalsOffset,
                                           IVulkanSceneCallbacks &callbacks) {
    ...
    graphics::VulkanRenderPass recorder(_options,
                                        _renderer.device(),
                                        _renderer.pipelines(),
                                        _renderer.uniformRing(),
                                        _renderer.descriptors(),
    ...
    recorder.setGlobalsOffset(globalsOffset);
    callbacks.draw(VulkanSceneDraw::Transparent, recorder);
 ...
 */
void VulkanScenePipeline::oitBlendPass(VkCommandBuffer cmd) {
 ...
 */
void VulkanScenePipeline::postProcessingPass(VkCommandBuffer cmd, uint32_t globalsOffset,
                                             IVulkanSceneCallbacks &callbacks) {
    drawOntoOutput(
        cmd, globalsOffset, VulkanSceneDraw::PostProcessing, "Post-processing", callbacks);
}

void VulkanScenePipeline::filterPass(VkCommandBuffer cmd,
 ...
 */
void VulkanScenePipeline::filterChainPass(VkCommandBuffer cmd) {
...
Texture &VulkanScenePipeline::render(const VulkanSceneFramePlan &plan,
    ...
    for (const auto step : plan.steps) {
        ...
        case VulkanSceneStep::Geometry:
            ...
            _spareImage = _ping.get();
            break;
        case VulkanSceneStep::RetroGeometry:
            retroGeometryPass(cmd, globalsOffset, callbacks);
            _frameImage = _output.get();
            _spareImage = _ping.get();
            break;
        case VulkanSceneStep::ScreenSpaceEffects:
            screenSpaceEffectsPass(cmd, globalsOffset);
            break;
        case VulkanSceneStep::Resolve:
            resolvePass(cmd, globalsOffset);
            break;
        case VulkanSceneStep::Transparency:
            transparencyPass(cmd, globalsOffset, callbacks);
            break;
        case VulkanSceneStep::OITBlend:
            oitBlendPass(cmd);
            break;
        case VulkanSceneStep::PostProcessing:
            postProcessingPass(cmd, globalsOffset, callbacks);
            break;
        case VulkanSceneStep::FilterChain:
            filterChainPass(cmd);
            break;
        }
    }
...
std::vector<VulkanScenePipeline::Target> VulkanScenePipeline::targetEntries(
    ...
    entries.push_back({"SSAO", "ssao", VulkanTargetKind::Color,
                       _ssao.get(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, false});
    entries.push_back({"SSR", "ssr", VulkanTargetKind::Color,
                       _ssr.get(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, false});
    entries.push_back({"Deferred opaque 2", "deferred_opaque_2", VulkanTargetKind::Color,
                       _hilights.get(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, false});
    entries.push_back({"OIT accum", "oit_accum", VulkanTargetKind::Color,
                       _oitAccum.get(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, false});
    entries.push_back({"OIT revealage", "oit_revealage", VulkanTargetKind::Color,

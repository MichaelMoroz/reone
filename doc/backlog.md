...
| 7.6 | Particle admission to the tracer is implemented but never observed | Landed with the G-buffer work, and produces no measurable traced change on any of six modules tried (danm14ab, ebo_m12aa, danm13, korr_m33aa, kas_m22aa, manm26aa): traced mean luminance moves by ~3e-6 with `--ptdenoise 0`, where the noise floor is 1e-6. Either none of them have emitters in view at frame 310, or admission does not work, and nothing currently distinguishes those. **Cheapest fix first: add grass, particle and billboard counts to the TLAS log line** (`rayquery.cpp`, the "TLAS N instances, Nk triangles" message), which already reports emissive, additive, saber, dangly and sky. Then find a scene with a live emitter and confirm the count is non-zero before trusting the image | P1 | S |

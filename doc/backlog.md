...
| # | Task | Why it matters | Pri | Eff |
|---|---|---|---|---|
| 7.1 | Scene nondeterminism blocks every image comparison | Measured 2026-07-31, `danm14ab` frame 310, two runs of one unmodified binary: retro raster **0.0245%** of pixels differ (mean abs 0.022), path tracing **13.40%** (mean abs 0.118), max channel delta 219 in both. Mean luminance agrees to three decimals either way, so the image is the same image and only individual pixels move. Present on every run, not the bimodal half-the-time effect originally recorded. Cost a false "nine captures failed" report on a change that touched no engine code. RNG, timing, SSAO/SSR, frame offset and threaded loading already ruled out. Next probes: constancy of `update` calls before the capture frame; any pass sampling an uncleared target. **Note the diagnostics skill still claims raster is byte-identical and tracing bounded at 0.02% — both false, fix that text too** | P1 | M |

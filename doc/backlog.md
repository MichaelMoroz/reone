...
| 1.12 | **Blended particles must not write the denoiser guides** | A smoke quad committed as the primary hit currently supplies viewZ, normal-roughness, motion and the demodulation factors for that pixel. All four are wrong: NRD reprojects from depth and motion, and a thin camera-facing quad drifting through the scene has neither the depth nor the motion of the surface actually being shaded behind it, so history is fetched from the wrong place and smears. Its albedo is not a meaningful demodulation factor either — the radiance at that pixel is mostly the geometry behind, divided by a guide belonging to the smoke. Visible as denoiser artefacts around the Dantooine vents, and it compounds the non-finite values in 1.13: a NaN written into a guide propagates across the frame through reprojection rather than staying in one pixel. **Guides should come from the first non-transmissive surface along the visibility ray**; transmissive hits contribute radiance and nothing else. That is also what makes the denoised result independent of how many quads happen to overlap | **P1** | M |
| 1.13 | Smoke particles produce non-finite pixels, and the denoiser spreads them | NaN on main-menu smoke, propagating across the frame once NRD reprojects it. Leading suspect is the unguarded `specularChannel += opacity * nee.specular / outputs.specFactor` in the blended-coverage branch of `rayquery.slang` — a rough, non-envmapped, non-metallic particle is exactly where a specular demodulation factor reaches zero. Ruled out already: the premultiplied-alpha flag is never set for these textures (`fx_smoke01`/`fx_smoke`, DXT5, `Blending::None`), the transmission cap is irrelevant (12 vs 96 pixel-identical), a plain alpha sign flip makes it worse, and `mainTex` resolves to a valid bindless id. Measure with `numpy.isfinite` on a `--dumptargets` `.npy`, not by eye — NaN renders as black, white or garbage depending on the path it takes | **P0** | M |
...
| 2.6 | **Geometric grass instead of alpha-tested quads** | Grass is punch-through quads, so in a traced frame every ray-triangle hit runs a candidate shader - interpolate UV, sample, compare, decide - which drops out of fixed-function traversal on exactly the geometry rays cross most. Measured on danm14ab at a grassy camera: 3x density costs +25% of the graphics slot, and it scales with samples (16 spp went 32.6 -> 46.4 ms). Opaque blade geometry stays in hardware, lets the BLAS build with `PREFER_FAST_TRACE` and compaction, and lets traversal commit and stop. Against it: 4-8x the triangles, a bigger BLAS, and a slower BLAS build - and grass clusters materialise as the camera moves, so that build cost is per frame rather than amortised. **Measure the ceiling before building anything.** Mark grass opaque and re-run: the image is wrong, but the frame time is the upper bound on what geometric grass can win, since it removes exactly the any-hit cost and adds none of the triangles. Two captures and a feature bit. **And do it after hybrid** (`vulkan-rt-backend.md` §11.2), which changes the answer: once raster owns primary visibility, grass is traced only for shadows and secondary bounces, so the primary rays - the ones crossing the most grass at the most pixels - stop existing and a large part of the cost goes away for free. A cheaper middle option exists either way: trim the quads to the opaque region of the texture, which cuts wasted candidate invocations with no new art | P2 | L |
...
| 4.9 | **Compile shaders in the engine with Slang, and drop the build step** | Today `slangc`/`dxc` transpile to SPIR-V at build time (`CMakeLists.txt:204`) and the engine loads the `.spv` from disk (`graphics/vulkan/renderer.cpp:71`). Link Slang instead and compile at startup and on demand: no shader step in the build, and editing a `.slang` shows up next frame. Nice to have rather than needed - the appeal is the iteration loop, not correctness. It does happen to retire the stale-module trap for free, since there is no longer a compiled artefact to lag the source. Wants: errors to the console instead of a crash, keep the last good module for a pipeline so a typo does not take the frame down, and a force-recompile. Startup cost is the thing to watch, since it moves from build time to every launch | P3 | M |
...
| 7.4 | Open design questions | Emitter granularity (per emitter vs per particle system); ~~whether walkmeshes and AABB debug geometry should register at all~~ *(answered: no — see 7.8)*; compute-skinning ownership; whether grass and particles are ever traced | P2 | — |

| 7.6 | ~~Particle admission unobserved~~ *(answered)* | It was working. The TLAS line now counts admitted grass clusters, particles and billboards, and `danm14ab` reports **1482 grass clusters, 55 particles** on the default camera. The earlier null result was a measurement failure, not a code failure: 55 saber-spark quads do not move a whole-frame mean luminance by more than 3e-6. Worth keeping as a lesson — whole-frame statistics cannot see small localized content, and a count answers in one run what six module probes could not |
| 7.7 | Isolation fixtures: grass cluster placement, not `scene empty` | Earlier entry claimed `scene empty` was broken because the registry logged `entries=0` and captures were black. **Both are correct behaviour** — an empty scene has no registered objects, the sky is baked at startup rather than being one, and a scene with no lights renders black. The commands work: `warp` + `scene empty` + `grass` admitted **2048 clusters** at the fixture's original 80-unit camera. Two real problems remain. Raster culls grass beyond `kMaxClusterDistance` = 32 (`node/grass.cpp:42`) while the tracer admits the whole pool, so at 80 units raster draws nothing and any comparison is meaningless. And moving the camera to 12 units produced **zero** clusters, which is the part worth diagnosing — cluster placement appears to depend on camera proximity to the surface in a way that fails at close range. Fixtures also need a light, or every capture is black by construction | P2 | S |
| 7.5 | **NRD accumulation is nondeterministic, and it was masquerading as tracer noise** | Isolated 2026-07-31 on `danm13` frame 310, `--dev 0`. Mean luminance over repeated runs: denoiser and FSR both off, sd **0.00063**; FSR alone on, sd **0.00051**; **NRD alone on, sd 0.14043** with one run 0.35 clear of the rest; both on, sd 0.01283 and bistable. So NRD is the whole of it, FSR contributes nothing, and FSR partly *masks* NRD's excursions. The tracer itself is deterministic in energy — six decimals — while per-pixel variation of 6-16% remains and is genuine MC and acceleration-structure ordering. Two consequences. **Measurement:** traced comparisons should run `--ptdenoise 0`, which turns a 16-runs-a-side statistical exercise back into a tight number. **Correctness:** REBLUR producing a different result from identical input is a real temporal-stability bug, not a harness artifact. The engine fixes the timestep and seeds the generator for capture runs, so suspect anything NRD is fed that is not frame-indexed — a wall-clock delta, a frames-since-reset counter that depends on load time, or a history-reset heuristic tripping on a timing-sensitive predicate | **P1** | M |

...
| 7.7 | Isolation fixtures produce an empty scene | `scene empty`, `spawn`, `grass`, `emit` and `ignite` exist and the traced G-buffer dump now works, but the fixtures in `test/fixtures/render-isolation` render nothing: the engine logs `Scene registry entries=0`, tracer depth is uniformly 0 and raster depth uniformly 2500 (the far plane). Almost certainly ordering — `warp` completes its module load some frames after the commands file has run, so `scene empty` and the spawn that follows it are applied against a scene that is then replaced. Until this works, no per-class comparison is possible, and any per-class numbers reported so far did not come from these fixtures | **P1** | M |

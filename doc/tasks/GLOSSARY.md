...
| **sky bake** | Rendering a sky shell into a cubemap by casting rays from inside it. **Two exist**: the offline `src/apps/skybake` (the intended survivor, still with no engine consumer) and the runtime one, now `Sky::bakeSkyRoom` (`src/libs/graphics/rendering/sky.cpp:79`, driven at `renderpipeline.cpp:198-207`) rather than `VulkanRayQuery::bakeSkyRoom` — the same bake, moved. V5 deletes the runtime one. Both raster resolves and the tracer now read the result through one `skyRadiance` body (`lib/skycube.slang:41`). | `DESIGN.md` |

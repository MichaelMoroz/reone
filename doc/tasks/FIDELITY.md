...
| 35 | Self-illumination operator | Retro multiplies: self-illum is summed with ambient and direct, clamped, then multiplied by the diffuse texture (`retro_resolve.slang:134-138`). Phase F's reference correction #6 says vanilla *adds* emission on top of the texture, citing KVP (`MaterialSystem.cpp:196`). But the references disagree — KotOR.js multiplies with a 0.25 floor (`ShaderOdysseyModel.ts:383`), and fixed-function `GL_MODULATE` computes `emission + ambient + Σdiffuse`, clamps, then multiplies the texture, which is exactly reone's expression. KVP's line sits inside its own legacy→PBR conversion and needs compensating albedo heuristics on the next lines. | **Reference-informed open** | [DESIGN.md](DESIGN.md) reference correction #6 | Do not treat correction #6 as settled either way. A `_lit`/`lsi_` pattern texture on a dark surface separates the two operators immediately. |
| 36 | Odyssey light falloff and range cull | Retro's range cull compares a linear distance against `radius²` (`retro_resolve.slang:116`) and its falloff is `radius²/(radius+d)²` rather than inverse-square. Both are dimensionally wrong and both are **deliberate**: backlog 1.9 records them as faithful ports of Odyssey behaviour, and `085c5893` corrected them in the tracer only. Decided 2026-08-05: **Retro keeps them; PBR does not.** `pbr_resolve.slang:83-85` and `:235` still carry the Odyssey versions with a comment saying they are kept as the GL shader had them. | **Intentional / accepted** for Retro; **confirmed unfinished design** for PBR | [DESIGN.md](DESIGN.md) **G6c** | Do not "fix" the retro light loop — a dimensional-analysis bug report there is expected and not actionable. G6c retires the Odyssey cull and falloff from PBR only, and any shared-lighting refactor must keep two functions rather than converging them. |
| 37 | MDL vertex colours | Ruled out as a difference. `MergedVertex.color` is a literal white constant for all MDL geometry (`scene_resolve.slang:301`), `SceneObject` carries no colour attribute offset, and the loader declares `offMdxVertexColors` (`mdlmdxreader.cpp:225`) and never reads it. Neither reference reads an MDX colour array either: xoreos's KotOR loader has none, and KotOR.js uses vertex colours only for walkmesh and path debug geometry. | **Ruled out** | — | No action. Note only that `diffuse *= input.color` (`scene_draw.slang:138`) is a guaranteed no-op for mesh geometry whose name implies a source that cannot exist. |
...
- **`DESIGN.md` misquotes the retail alpha test** as
  ...
  (`graphics/graphics.cpp:440`). The G8 passage at `DESIGN.md-852` that

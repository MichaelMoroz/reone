...
| RAS-003 | **G8 — the blended pass and the three alpha kinds** | open | P1 | L | phase-f G8 | — | DESIGN.md; backlog 5.4-adjacent. Tracer side is gated on the same work: `lib/megadraw_geometry.slang:58-60` publishes cutout coverage only, "blended and additive surfaces contribute nothing until G8", in lockstep with `tracing/trace.slang`'s candidateAlpha/commitsCoverage |
...
| RAS-005 | G8 — per-node alpha fade must actually render | open | P1 | M | phase-f G8 | RAS-004 | FIDELITY #18; `scene_resolve.slang:301` hard-codes vertex colour white |
...
| RAS-015 | Adopt textured roughness/metalness in one step for both consumers | open | P2 | M | none | RAS-001 | DESIGN.md; both currently derive roughness from diffuse alpha. Shader sites, which must move together: `pbr_resolve.slang:163-166` (metallic 0, roughness = clamped diffuse alpha), `lib/surface.slang:36-39` (the same stand-ins in the shared surface struct), `tracing/material.slang:219` ("keep the tracer's stand-ins exactly aligned with pbr_resolve") |
...
| RAS-019 | SSAO — unowned, decide | open | P3 | M | none | — | DESIGN.md; screen-space approximation of what PT computes properly. The resolve already has the seam: `pbr_resolve.slang:167` hard-codes `ao = 1.0` with "SSAO is not part of the shared executor yet", and `pbr_ssao.slang` exists unwired |
...
| RAS-024 | Settings exposes the pipeline's shape, not the three render modes | open | P2 | S | none | — | 2026-08-06, user-spotted. Retro-vs-PBR is a live `PBR resolve` checkbox under General while `Render mode` (Raster/Path tracing) is a staged combo under Requires-reapply, so choosing between the three modes a user actually has - retro, PBR, path-traced - means knowing that two of them are a boolean and the third is a rebuild. Collapse to one three-way Render mode control: Retro and PBR apply live, Path tracing stages for Apply, and the reapply note shows only for that choice. `mode` and `pbr` stay separate in GraphicsOptions and as CLI flags - scripts and configs depend on them - so this is a UI and console front door (`gfx set rendermode retro|pbr|path-tracing`), not an options rename. Same mistake as the earlier as-is merge of the path-tracing window: exposing the machinery's shape rather than the user's |
...
| TOOL-025 | NRD shader blobs can be stale/empty and CMake accepts them | open | **P0** | S | none | — | 2026-08-05; an empty 67-byte `NVSP` blob for `Clear.cs.hlsl` killed path tracing twice and read as an RHI regression; `vkCreateShaderModule`'s `VkResult` is discarded at `nrddenoiser.cpp:203` — RECORD.md 1.16; recurred 3× — the one stale file is `build/_deps/nrd-src/_Shaders/Clear.cs.spirv.h` at 67 bytes of 31 headers; `rm` it and rebuild regenerates 3,107 bytes |

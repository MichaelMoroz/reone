...
| 19 | Alpha-test reference value | Reone tests at **0.5** (`lib/megadraw_geometry.slang:11`, applied `:57`), in the gated G-buffer draw and in **both shadow passes** (`shadow_megadraw.slang:43`, `:79`). xoreos sets `glAlphaFunc(GL_GREATER, 0.1f)` once globally (`graphics/graphics.cpp:440-441`) and only ever disables it around the env-map pass. **`phase-f.md:1338` misquotes this as `GL_GREATER, 0`** — the authority is 0.1, so both the "conflict is a wording slip" reasoning and the defence of 0.5 at `phase-f.md:847-852` rest on a value the reference does not use. The kill band is 5× retail's on every foliage card, fence, and grille, and the same threshold thins their shadows. | **Reference-informed open** | [Phase F](phase-f.md) **G8** coverage rule | Correct the quoted reference value first, then capture a foliage silhouette at 0.1 and 0.5 side by side before choosing. This is a preservation question, not a taste one. |
...
| 26 | No shadow or soft-shadow option | `GraphicsOptions` exposes only `shadowOpacity` and `shadowResolution`; the launcher offers resolution alone (`frame.cpp:188-198`). Retail shipped both `Shadows` and `Soft Shadows` toggles — see the retail ini KVP reproduces (`swkotor.ini.example:93-94`). Retro also uses four-cascade CSM with a rotated 16-tap Poisson kernel (`lib/shadow.slang`) unconditionally, a technique the original did not have at all. | **Source divergence, unproven** | [Phase F](phase-f.md) G7 follow-on | The technique gap is acknowledged in Phase F as a modern reconstruction with no reference. The missing *options* are separable and cheap; decide whether Retro exposes them. |

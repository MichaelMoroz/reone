...
public:
    ...
    void executeDrawParticles(graphics::Material &material,
                              ...
                              const std::vector<VulkanParticleInstance> &particles);

    void executeDrawGrass(float radius,
                          float quadSize,
                          graphics::Material &material,
                          const std::vector<VulkanGrassInstance> &instances);

    void setShadowKind(ShadowKind kind) { _shadowKind = kind; }
...
private:
    ...
    Kind _kind;
    ShadowKind _shadowKind {ShadowKind::None};
